...
TEST_CASE("qtcount::basic ctor render", "[weight=1][part=qtcount]") {
    ...
    out.writeToFile("images/out/geo_out.png");
...
TEST_CASE("qtcount::basic prune", "[weight=1][part=qtcount]") {
    ...
    qtcount t1(img);
    PNG initial_pic = t1.render();
    initial_pic.writeToFile("images/out/out_adasquare.png");

    t1.prune(3000);
    PNG result = t1.render();

    result.writeToFile("images/out/out-adaPrune-count.png");
